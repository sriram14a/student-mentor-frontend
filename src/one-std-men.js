...
export function OneStudentOneMentor() {
  ...
  async function CreateStudentMentor(event) {
    ...
    const response = await fetch(`${API}/onestudentmentor`, {
      method: "POST",
    ...
    <div className="mt-4">
      <div className="d-flex flex-wrap justify-content-around">
        <div className="width-1">
          <h1> Assign ONE student to ONE mentor</h1>
        ...
        <div className="width-1">
          <h3 className="p-2 bg-success text-light rounded">
            One Student to One Mentor & change mentor
          </h3>
          <div>
              ...
              <tbody>
                {response.map((re) => (
                  <tr key={re.id}>
                    ...
                    <td>{re.mentorname}<IconButton
                    color="error"
                    aria-label="deleteButton"
                    onClick={() => { navigate(`/onestudentmentor/${re.id}`) }}
                  >
