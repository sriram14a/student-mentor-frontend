...
export function EditoneMentor1() {
  const { mentorid } = useParams();
  const [mentor, setMentor] = useState(null);

  useEffect(() => {
    fetch(`${API}/onestudentmentor/${mentorid}`, {
      method: "GET",
    })
      .then((data) => data.json())
      .then((data) => {
  ...
  return mentor ? <EditMentorForm2 mentor={mentor} /> : "Loading.....";
}

function EditMentorForm2({ mentor }) {
  ...
  async function Edited() {
    const response = await fetch(
      `${API}/onestudentmentor/${mentor.id}`,
      {
        method: "PUT",
